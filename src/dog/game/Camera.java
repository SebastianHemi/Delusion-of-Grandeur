...
public class Camera {
   ...
   public Camera(GameContainer gc, TiledMap map) {
      ...
      this.numTilesX = map.getWidth();	//number of displayed tiles
      this.numTilesY = map.getHeight();
      
      this.tileWidth = map.getTileWidth();	//size of 1 tile in pixel
      this.tileHeight = map.getTileHeight();
      
      this.mapHeight = this.numTilesX * this.tileWidth;	//size of map in pixel
   ...
   /**
    * "locks" the camera on the center of the given GameObject. The camera tries to keep the location in it's center.
    * @param gobj the GameObject which should be centered on the screen
    */
   public void centerOn(GameObject gobj) {
	      this.centerOn(gobj.getPosX() + gobj.getWidth()/2, gobj.getPosY() + gobj.getHeight()/2);
	   }
